...
void OpenRGBDialog2::AddClientTab()
{
    ...
    if(ClientInfoPage == NULL)
    {
        ...
        ui->MainTabBar->insertTab(1, ClientInfoPage, tr("SDK Client"));
    }
}
...
void OpenRGBDialog2::AddServerTab()
{
    ...
    ui->MainTabBar->insertTab(1, ServerInfoPage, tr("SDK Server"));
}

void OpenRGBDialog2::AddHWKeyboardTab()
{
    /*-----------------------------------------------------*\
    | Add HelloWordDFUPage tab if there is a HelloWordDFUPage|
    \*-----------------------------------------------------*/
    if(showHelloWordFWUpdate){
        helloWordDFUPage = new Ui::HelloWordDFUPage();
        helloWordDFUPage->setObjectName(QString("HelloWord Keyboard Updater"));
        ui->MainTabBar->insertTab(0, helloWordDFUPage, tr("HelloWord Keyboard Updater"));
        ui->MainTabBar->setCurrentIndex(0);
    }else{

        ui->MainTabBar->removeTab(0);
    }
    if(!showHelloWordFWUpdate){
        helloWordDFUPage == nullptr;
    }
    showHelloWordFWUpdate = !showHelloWordFWUpdate;
...
| 打开瀚文固件升级界面                                      |
\*-----------------------------------------------------*/
void Ui::OpenRGBDialog2::on_pushButton_clicked()
{
    AddHWKeyboardTab();
}
